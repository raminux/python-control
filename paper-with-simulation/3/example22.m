...
%             S = M_new*M_new';
    ...
    Thetahatf1_SGD(:, k+1) = Thetahatf1_SGD(:, k) - blkdiag(eye(2), zeros(2))*(EG_SGD*phi(:, k)*e1_SGD(k));
    Thetahatf2_SGD(:, k+1) = Thetahatf2_SGD(:, k) - blkdiag(0, eye(3))*(EG_SGD*phi(:, k)*e2_SGD(k));

    % ThetaTilda for SCL
    ThetaTilda_SCL_F1_norm(1, k) = norm([Thetahatf1(1,k)-Thetaf1(1), Thetahatf1(2, k)-Thetaf1(2)]);
    ThetaTilda_SCL_F2_norm(1, k) = norm([Thetahatf2(2,k)-Thetaf2(1), Thetahatf2(4, k)-Thetaf2(3)]);
    ThetaTilda_SCL_G2_norm(1, k) = norm(Thetahatf2(3,k)-Thetaf2(2));

    % Fuction approximation error integral for SCL
    F1hat = Thetahatf1(1, k)*X1 + Thetahatf1(2, k)*X2;
    F2hat = Thetahatf2(2, k)*X2 + Thetahatf2(4, k)*sin(X1);
    G2hat = Thetahatf2(3, k)*X1;
    F1 = Thetaf1(1)*X1 + Thetaf1(2)*X2;
    F2 = Thetaf2(1)*X2 + Thetaf2(3)*sin(X1);
    G2 = Thetaf2(2)*X1;
    E_SCL(1, k) = trapz(x2_trapz, trapz(x1_trapz, abs(F1hat-F1), 2));
    E_SCL(2, k) = trapz(x2_trapz, trapz(x1_trapz, abs(F2hat-F2), 2));
    E_SCL(3, k) = trapz(x2_trapz, trapz(x1_trapz, abs(G2hat-G2), 2));

    % Fuction approximation error integral for SGD
    F1hat_SGD = Thetahatf1_SGD(1, k)*X1 + Thetahatf1_SGD(2, k)*X2;
    F2hat_SGD = Thetahatf2_SGD(2, k)*X2 + Thetahatf2_SGD(4, k)*sin(X1);
    G2hat_SGD = Thetahatf2_SGD(3, k)*X1;
    E_SGD(1, k) = trapz(x2_trapz, trapz(x1_trapz, abs(F1hat_SGD-F1), 2));
    E_SGD(2, k) = trapz(x2_trapz, trapz(x1_trapz, abs(F2hat_SGD-F2), 2));
    E_SGD(3, k) = trapz(x2_trapz, trapz(x1_trapz, abs(G2hat_SGD-G2), 2));
    ...
    Thetahatf2_SCL_3(ins, :) = Thetahatf2(4, :);
    ...
    Thetahatf2_SGD_3(ins, :) = Thetahatf2_SGD(4, :);
    ...
    ThetaTilda_SCL_G2(i, :) = ThetaTilda_SCL_G2_norm;
...
ThetaTilda_SCL_G2_min = min(ThetaTilda_SCL_G2);
ThetaTilda_SCL_G2_max = max(ThetaTilda_SCL_G2);
ThetaTilda_SCL_G2_mean = mean(ThetaTilda_SCL_G2);
...
E_SCL_3_min = min(E_SCL_3);
E_SCL_3_max = max(E_SCL_3);
E_SCL_3_mean = mean(E_SCL_3);
...
E_SGD_3_min = min(E_SGD_3);
E_SGD_3_max = max(E_SGD_3);
E_SGD_3_mean = mean(E_SGD_3);
...
Thetahatf2_SCL_3_min = min(Thetahatf2_SCL_3);
Thetahatf2_SCL_3_max = max(Thetahatf2_SCL_3);
Thetahatf2_SCL_3_mean = mean(Thetahatf2_SCL_3);
...
Thetahatf2_SGD_3_min = min(Thetahatf2_SGD_3);
Thetahatf2_SGD_3_max = max(Thetahatf2_SGD_3);
Thetahatf2_SGD_3_mean = mean(Thetahatf2_SGD_3);
...
if ivb == 1 
    subplot(2, 1, 1)
    p_scl=fill([1:N+1, N+1:-1:1], [E_SGD_3_max, E_SGD_3_min(end:-1:1)],'r');
    p_scl.FaceColor = [1 0.8 0.8];      
    p_scl.EdgeColor = 'none';
    hold on 
    plt_sgd = plot(1:N+1,E_SGD_3_mean,'-.r');
    p_scl = fill([1:N+1, N+1:-1:1], [E_SCL_3_max, E_SCL_3_min(end:-1:1)],'b');
    p_scl.FaceColor = [0.8 0.8 1];      
    p_scl.EdgeColor = 'none';
    plt_scl = plot(1:N+1,E_SCL_3_mean,'--b');
    legend([plt_scl, plt_sgd], {'$CL-SGD,\sigma^2=3 \times 10^{-7} $','$SGD,\sigma^2=3 \times 10^{-7} $'},'Interpreter','latex')
    ylabel({'$E_{g_2}(k)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
else 
    subplot(2, 1, 2)
    p_scl=fill([1:N+1, N+1:-1:1], [E_SGD_3_max, E_SGD_3_min(end:-1:1)],'r');
...
if ivb == 1
    ...
    ylabel({'$F_1 - Parameters$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
else 
    subplot(2, 1, 2)
    p_scl=fill([1:N+1, N+1:-1:1], [Thetahatf1_SCL_1_max, Thetahatf1_SCL_1_min(end:-1:1)],'b');
...
if ivb == 1
    ...
    p_sgd.FaceColor = [1 0.8 0.8];      
    p_sgd.EdgeColor = 'none';
    p_scl=fill([1:N+1, N+1:-1:1], [Thetahatf2_SCL_3_max, Thetahatf2_SCL_3_min(end:-1:1)],'b');
    hold on 
    plt_scl = plot(1:N+1,Thetahatf2_SCL_3_mean,'--b');
    p_scl.FaceColor = [0.8 0.8 1];      
    p_scl.EdgeColor = 'none';
    p_sgd=fill([1:N+1, N+1:-1:1], [Thetahatf2_SGD_3_max, Thetahatf2_SGD_3_min(end:-1:1)],'r');
    hold on 
    plt_sgd = plot(1:N+1,Thetahatf2_SGD_3_mean,'-.r');
    p_sgd.FaceColor = [1 0.8 0.8];      
    p_sgd.EdgeColor = 'none';
    plt_true = plot(1:N+1, Thetaf2(1)*ones(1, N+1),'k','LineWidth',1.);
    hold on
    plt_true = plot(1:N+1, Thetaf2(3)*ones(1, N+1),'k','LineWidth',1.);
    legend([plt_true, plt_scl, plt_sgd], {'$True$', '$CL-SGD, \sigma^2=3 \times 10^{-7}$','$SGD, \sigma^2=3 \times 10^{-7}$'},'Interpreter','latex')
...
if ivb == 1
    ...
    legend([plt_true, plt_scl, plt_sgd], {'$True$', '$CL-SGD, \sigma^2=3 \times 10^{-7}$','$SGD, \sigma^2=3 \times 10^{-7}$'},'Interpreter','latex')
    ylabel({'Parameters of $g_2(x_k)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
else
    subplot(2, 1, 2)
    p_scl=fill([1:N+1, N+1:-1:1], [Thetahatf2_SCL_2_max, Thetahatf2_SCL_2_min(end:-1:1)],'b');
...
if ivb == 1
    subplot(2, 1, 1)
    p_scl=fill([1:N+1, N+1:-1:1], [ThetaTilda_SCL_G2_max, ThetaTilda_SCL_G2_min(end:-1:1)],'b');
    hold on 
    plt_scl = plot(1:N+1,ThetaTilda_SCL_G2_mean,'--b');
    p_scl.FaceColor = [0.8 0.8 1];      
    p_scl.EdgeColor = 'none';
    plt_true = plot(1:N+1, 1*ones(1, N+1),'k','LineWidth',1.); % TODO: Change 1 to sqrt(b/eps)
    legend([plt_true, plt_scl], {'$\sqrt{\frac{b}{\epsilon}}$', '$CL-SGD, \sigma^2=3 \times 10^{-7}$'},'Interpreter','latex')
    ylabel({'$\tilde \Theta_{g_2}(k)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
