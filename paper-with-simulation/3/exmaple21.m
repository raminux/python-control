...
%% High Order Tuner (HOT) Parameters
gamma = 0.4;
beta = 0.2;
mu = 0.001;
...
% HOT
X1_HOT = zeros(Num_Sim, N+1);
X2_HOT = zeros(Num_Sim, N+1);

X1_HOT_min = zeros(1, N+1);
X1_HOT_max = zeros(1, N+1);
X1_HOT_mean = zeros(1, N+1);

X2_HOT_min = zeros(1, N+1);
X2_HOT_max = zeros(1, N+1);
X2_HOT_mean = zeros(1, N+1);

Thetahatf1_HOT_1 = zeros(Num_Sim, N+1);
Thetahatf1_HOT_1_min = zeros(1, N+1);
Thetahatf1_HOT_1_max = zeros(1, N+1);
Thetahatf1_HOT_1_mean = zeros(1, N+1);

Thetahatf1_HOT_2 = zeros(Num_Sim, N+1);
Thetahatf1_HOT_2_min = zeros(1, N+1);
Thetahatf1_HOT_2_max = zeros(1, N+1);
Thetahatf1_HOT_2_mean = zeros(1, N+1);

Thetahatf2_HOT_1 = zeros(Num_Sim, N+1);
Thetahatf2_HOT_1_min = zeros(1, N+1);
Thetahatf2_HOT_1_max = zeros(1, N+1);
Thetahatf2_HOT_1_mean = zeros(1, N+1);

Thetahatf2_HOT_2 = zeros(Num_Sim, N+1);
Thetahatf2_HOT_2_min = zeros(1, N+1);
Thetahatf2_HOT_2_max = zeros(1, N+1);
Thetahatf2_HOT_2_mean = zeros(1, N+1);

E_HOT = zeros(3, N+1); % 3 for three functions f1, f2, g2

E_HOT_1 = zeros(Num_Sim, N+1); %f1
E_HOT_1_min = zeros(1, N+1);
E_HOT_1_max = zeros(1, N+1);
E_HOT_1_mean = zeros(1, N+1);

E_HOT_2 = zeros(Num_Sim, N+1); %f2
E_HOT_2_min = zeros(1, N+1);
E_HOT_2_max = zeros(1, N+1);
E_HOT_2_mean = zeros(1, N+1);
...
for ins=1:Num_Sim
    ...
    % High Order Tuner Algorithm
    Thetahatf1_HOT = zeros(q, N);
    Thetahatf2_HOT = zeros(q, N);
    vu1 = zeros(q, N);
    vu2 = zeros(q, N);
    Thetahatf1_HOT_bar = zeros(q, N);
    Thetahatf2_HOT_bar = zeros(q, N);
    ...
    for k=1:N
        ...
        % Fuction approximation error integral for HOT
        F1hat_HOT = Thetahatf1_HOT(1, k)*X1 + Thetahatf1_HOT(2, k)*X2;
        F2hat_HOT = Thetahatf2_HOT(2, k)*X2 + Thetahatf2_HOT(3, k)*sin(X1);
        E_HOT(1, k) = trapz(x2_trapz, trapz(x1_trapz, abs(F1hat_HOT-F1), 2));
        E_HOT(2, k) = trapz(x2_trapz, trapz(x1_trapz, abs(F2hat_HOT-F2), 2));
        ...
        if k<=N/4
            u(k) = -0.7*x1(k)-0.7*x2(k);
        elseif N/4<k && k<=N/2
            u(k) = -0.5*x1(k) - 0.5*x2(k);
        else
            u(k) = -0.4*x1(k)-0.4*x2(k);
        end
           

%         u(k) = -0.8*x1(k)-0.8*x2(k) + 0*exp(-1*k)*(sin(0.1*x1(k))+.5*sin(0.3*x2(k)) ...
%             +0.1*sin(0.7*x1(k))+0.8*sin(1.6*x2(k))+0.2*sin(1.4*x2(k)));
    ...
    %         +0.1*sin(0.7*x1(k))+0.8*sin(1.6*x2(k))+0.2*sin(1.4*x2(k)));
        ...
        e1_SGD(k) = yh1_SGD(k) - (y1(k)+2*v1(k));
        e2_SGD(k) = yh2_SGD(k) - (y2(k)+2*v2(k));
...
%     rank(M)
    ...
    %             S = M_new*M_new';
        ...
        % HOT implementation
        Nk = 1 + norm(phi(:, k));
        Thetahatf1_HOT_bar(:, k) = Thetahatf1_HOT(:, k) - gamma*beta*blkdiag(eye(2), 0)*(phi(:, k)*(Thetahatf1_HOT(:, k)'*phi(:, k)-(y1(k)+2*v1(k)))/Nk + mu*(Thetahatf1_HOT(:, k)-[0;0;0]));
        Thetahatf1_HOT(:, k+1) = Thetahatf1_HOT_bar(:, k) - beta*blkdiag(eye(2), 0)*(Thetahatf1_HOT_bar(:, k)-vu1(:, k));
        vu1(:, k+1) = vu1(:, k) - gamma*blkdiag(eye(2), 0)*(phi(:, k)*(Thetahatf1_HOT(:, k+1)'*phi(:, k)-(y1(k)+2*v1(k)))/Nk + mu*(Thetahatf1_HOT(:, k+1)-[0;0;0]));

        Thetahatf2_HOT_bar(:, k) = Thetahatf2_HOT(:, k) - gamma*beta*blkdiag(0, eye(2))*(phi(:, k)*(Thetahatf2_HOT(:, k)'*phi(:, k)-(y2(k)+2*v2(k)))/Nk + mu*(Thetahatf2_HOT(:, k)-[0;0;0]));
        Thetahatf2_HOT(:, k+1) = Thetahatf2_HOT_bar(:, k) - beta*blkdiag(0, eye(2))*(Thetahatf2_HOT_bar(:, k)-vu2(:, k));
        vu2(:, k+1) = vu2(:, k) - gamma*blkdiag(0, eye(2))*(phi(:, k)*(Thetahatf2_HOT(:, k+1)'*phi(:, k)-(y2(k)+2*v2(k)))/Nk + mu*(Thetahatf2_HOT(:, k+1)-[0;0;0]));
    ...
    ThetaTilda_SCL_F1(ins, :) = ThetaTilda_SCL_F1_norm;
    ThetaTilda_SCL_F2(ins, :) = ThetaTilda_SCL_F2_norm;

    % HOT 
    Thetahatf1_HOT_1(ins, :) = Thetahatf1_HOT(1, :);
    Thetahatf1_HOT_2(ins, :) = Thetahatf1_HOT(2, :);
    Thetahatf2_HOT_1(ins, :) = Thetahatf2_HOT(2, :);
    Thetahatf2_HOT_2(ins, :) = Thetahatf2_HOT(3, :);
...
% HOT
Thetahatf1_HOT_1_min = min(Thetahatf1_HOT_1);
Thetahatf1_HOT_1_max = max(Thetahatf1_HOT_1);
Thetahatf1_HOT_1_mean = mean(Thetahatf1_HOT_1);

Thetahatf1_HOT_2_min = min(Thetahatf1_HOT_2);
Thetahatf1_HOT_2_max = max(Thetahatf1_HOT_2);
Thetahatf1_HOT_2_mean = mean(Thetahatf1_HOT_2);

Thetahatf2_HOT_1_min = min(Thetahatf2_HOT_1);
Thetahatf2_HOT_1_max = max(Thetahatf2_HOT_1);
Thetahatf2_HOT_1_mean = mean(Thetahatf2_HOT_1);

Thetahatf2_HOT_2_min = min(Thetahatf2_HOT_2);
Thetahatf2_HOT_2_max = max(Thetahatf2_HOT_2);
Thetahatf2_HOT_2_mean = mean(Thetahatf2_HOT_2);

E_HOT_1_min = min(E_HOT_1);
E_HOT_1_max = max(E_HOT_1);
E_HOT_1_mean = mean(E_HOT_1);

E_HOT_2_min = min(E_HOT_2);
E_HOT_2_max = max(E_HOT_2);
E_HOT_2_mean = mean(E_HOT_2);
...
if ivb == 1
    ...
    p_scl = fill([1:N+1, N+1:-1:1], [E_HOT_1_max, E_HOT_1_min(end:-1:1)],'g');
    p_scl.FaceColor = [0.8 1 0.8];      
    p_scl.EdgeColor = 'none';
    plt_hot = plot(1:N+1,E_HOT_1_mean,'g');
    legend([plt_hot, plt_scl, plt_sgd], {'$HOT, \sigma^2= 10^{-6}$', '$CL-SGD, \sigma^2= 10^{-6}$','$SGD,\sigma^2= 10^{-6} $'},'Interpreter','latex')
    ...
    p_scl.EdgeColor = 'none';
    plt_hot = plot(1:N+1,E_HOT_1_mean,'g');
    legend([plt_hot, plt_scl, plt_sgd], {'$HOT,\sigma^2= 10^{-4} $', '$CL-SGD,\sigma^2= 10^{-4} $','$SGD,\sigma^2= 10^{-4} $'},'Interpreter','latex')
    ylabel({'$E_{f_1}(k)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
end
...
if ivb == 1
    ...
    p_scl = fill([1:N+1, N+1:-1:1], [E_HOT_2_max, E_HOT_2_min(end:-1:1)],'g');
    p_scl.FaceColor = [0.8 1 0.8];      
    p_scl.EdgeColor = 'none';
    plt_hot = plot(1:N+1,E_HOT_2_mean,'g');
    legend([plt_hot, plt_scl, plt_sgd], {'$HOT,\sigma^2= 10^{-6} $', '$CL-SGD,\sigma^2= 10^{-6} $','$SGD,\sigma^2= 10^{-6} $'},'Interpreter','latex')
...
if ivb == 1
    ...
    p_sgd=fill([1:N+1, N+1:-1:1], [Thetahatf1_HOT_1_max, Thetahatf1_HOT_1_min(end:-1:1)],'g');
    hold on 
    plt_hot = plot(1:N+1,Thetahatf1_HOT_1_mean,'g');
    p_sgd.FaceColor = [0.8 1 0.8];      
    p_sgd.EdgeColor = 'none';
    ...
    p_sgd.FaceColor = [1 0.8 0.8];      
    p_sgd.EdgeColor = 'none';
    p_sgd=fill([1:N+1, N+1:-1:1], [Thetahatf1_HOT_2_max, Thetahatf1_HOT_2_min(end:-1:1)],'g');
    hold on 
    plt_hot = plot(1:N+1,Thetahatf1_HOT_2_mean,'g');
    p_sgd.FaceColor = [0.8 1 0.8];      
    p_sgd.EdgeColor = 'none';
    plt_true = plot(1:N+1, Thetaf1(1)*ones(1, N+1),'k','LineWidth',1.);
    hold on
    plt_true = plot(1:N+1, Thetaf1(2)*ones(1, N+1),'k','LineWidth',1.);
    legend([plt_true, plt_hot, plt_scl, plt_sgd], {'$True$', '$HOT, \sigma^2= 10^{-6}$', '$CL-SGD, \sigma^2= 10^{-6}$','$SGD, \sigma^2= 10^{-6}$'},'Interpreter','latex')
    ylabel({'Parameters of $f_1(x)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
else 
    subplot(2, 1, 2)
    p_scl=fill([1:N+1, N+1:-1:1], [Thetahatf1_SCL_1_max, Thetahatf1_SCL_1_min(end:-1:1)],'b');
    ...
    hold on
    plt_true = plot(1:N+1, Thetaf1(2)*ones(1, N+1),'k','LineWidth',1.);
    legend([plt_true, plt_hot, plt_scl, plt_sgd], {'$True$', '$HOT, \sigma^2= 10^{-4}$', '$CL-SGD, \sigma^2= 10^{-4}$','$SGD, \sigma^2= 10^{-4}$'},'Interpreter','latex')
    ylabel({'Parameters of $f_1(x)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
end 
...
if ivb == 1
    ...
    p_sgd.FaceColor = [1 0.8 0.8];      
    p_sgd.EdgeColor = 'none';
    p_sgd=fill([1:N+1, N+1:-1:1], [Thetahatf2_HOT_1_max, Thetahatf2_HOT_1_min(end:-1:1)],'g');
    hold on 
    plt_hot = plot(1:N+1,Thetahatf2_HOT_1_mean,'g');
    p_sgd.FaceColor = [0.8 1 0.8];      
    p_sgd.EdgeColor = 'none';
    ...
    p_sgd.FaceColor = [1 0.8 0.8];      
    p_sgd.EdgeColor = 'none';
    p_sgd=fill([1:N+1, N+1:-1:1], [Thetahatf2_HOT_2_max, Thetahatf2_HOT_2_min(end:-1:1)],'g');
    hold on 
    plt_hot = plot(1:N+1,Thetahatf2_HOT_2_mean,'g');
    p_sgd.FaceColor = [0.8 1 0.8];      
    p_sgd.EdgeColor = 'none';
    ...
    legend([plt_true, plt_hot, plt_scl, plt_sgd], {'$True$', '$HOT, \sigma^2= 10^{-6}$', '$CL-SGD, \sigma^2= 10^{-6}$','$SGD, \sigma^2= 10^{-6}$'},'Interpreter','latex')
    ylabel({'Parameters of $f_2(x)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
else
    subplot(2, 1, 2)
    p_scl=fill([1:N+1, N+1:-1:1], [Thetahatf2_SCL_1_max, Thetahatf2_SCL_1_min(end:-1:1)],'b');
...
if ivb == 1
    ...
    legend([plt_true, plt_scl], {'$\sqrt{\frac{b}{\epsilon}}$', '$CL-SGD, \sigma^2= 10^{-6}$'},'Interpreter','latex')
    ...
    p_scl.EdgeColor = 'none';
    plt_true = plot(1:N+1, 1*ones(1, N+1),'k','LineWidth',1.); % TODO: Change 1 to sqrt(b/eps)
    legend([plt_true, plt_scl], {'$\sqrt{\frac{b}{\epsilon}}$', '$CL-SGD, \sigma^2= 10^{-4}$'},'Interpreter','latex')
    ylabel({'$\tilde \Theta_{f_1}(k)$'}, 'Interpreter', 'latex')
    xlabel('k (time steps)')
end
